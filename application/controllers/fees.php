...
class Fees_Controller extends Controller
{
	...
	public function add($fee_type_id = NULL)
	{
		...
		if ($fee_type_id && is_numeric($fee_type_id)) {
			...
			$fee_type = $enum_type_model->where(
				array(
					'id'		=> $fee_type_id,
	...
	public function edit($fee_id = NULL)
	{
		...
		$form->date('from')
			...
			->callback(array($this, 'valid_interval'))
			->label('Date from')
			->value(strtotime((string)($fee->from ?? '')));

		$form->date('to')
			->rules('required')
			->label('Date to')
			->value(strtotime((string)($fee->to ?? '')));
	...
	public function valid_interval($input = NULL)
	{
		...
		$arr_from = date_parse_from_format(DateTime::ATOM, $this->input->post('from'));
		$arr_to = date_parse_from_format(DateTime::ATOM, $this->input->post('to'));
