...
function site_lang()
{
	...
	if (!array_key_exists($lang, $locales)) {
		...
		url::redirect(
			'/' . trim($index_page . $lang . '/' . ltrim(url_lang::current(), '/'), '/')
		);
	}
	...
	$langLocale = Config::get('language');
	if (setlocale(
		LC_ALL,
		$langLocale . '.UTF-8',
		$langLocale . '.utf8',
		...
		throw new ErrorException(
			'Cannot setlocale to: ' . $langLocale . '.UTF-8 (or utf8). ' .
				'Please generate locale in your system.'
		);
	}

	// Decimal number are always written with .
	// Decimal numbers are always written with .
	if (setlocale(LC_NUMERIC, 'en_US.UTF-8', 'en_US.utf8', 'C') === FALSE) {
		throw new ErrorException(
			'Cannot setlocale to: en_US.UTF-8 (or en_US.utf8). ' .
				'Please generate locale in your system.'
		);
	}
